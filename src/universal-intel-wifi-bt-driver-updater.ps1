...
.RELEASENOTES
v2026.03.0003 - Rebuilt Parse-WiFiDownloadList to support per-DEV blocks (mirrors BT parser).
               Enables per-device CAB and version/date for legacy Wi-Fi devices.
               intel-wifi-drivers-download.txt format updated (backward-compatible).
               Added Microsoft WHCP digital signature verification for CAB packages before installation. Added per-device version/date support in database files for legacy device handling (e.g. AX200 BT PID_0029). Backward-compatible with v2026.03.0002 database format.
v2026.03.0002 - Initial release aligned with Universal Intel Chipset Device Updater architecture
#>
    ...
    try {
        ...
        if ($comparisonResult -eq 0) {
            ...
            if ($downloadChoice -eq 'Y') {
                $downloadUrl = "https://github.com/FirstEverTech/Universal-Intel-WiFi-BT-Updater/releases/download/v$latestVersion/WiFiBT-Updater-$latestVersion-Win10-Win11.exe"
                $downloadsFolder = Get-DownloadsFolder
                $outputPath = Join-Path $downloadsFolder "WiFiBT-Updater-$latestVersion-Win10-Win11.exe"
                ...
                try {
                    ...
                    Write-Host " Download completed. Please run the new version." -ForegroundColor Green
                } catch {
                    Write-Host " Download failed: $($_.Exception.Message)" -ForegroundColor Red
                    Write-Host " Please visit: https://github.com/FirstEverTech/Universal-Intel-WiFi-BT-Updater/releases" -ForegroundColor Cyan
                }
            }
    ...
    try {
        ...
        foreach ($line in $lines) {
            ...
            if ($line -match '^Latest Version\s*=\s*([0-9.]+)') {
                ...
                # Backward-compatible: old rows with 4 columns remain valid,
                # new rows with 6 columns carry device-specific version and date.
                $deviceVersion = $null
                $deviceDate    = $null
                ...
                $result.Devices[$devId] = @{
                    ...
                    Models        = $models
                    Generation    = $gen
                    LatestVersion = $deviceVersion   # $null = use global
                    ReleaseDate   = $deviceDate      # $null = use global
                }
    ...
    try {
        ...
        foreach ($line in $lines) {
            ...
            if ($line -match '^Latest Version\s*=\s*([0-9.]+)') {
                ...
                # Optional per-device columns 5 & 6: Latest Version | Release Date
                # Backward-compatible: old rows with 4 columns remain valid,
                # new rows with 6 columns carry device-specific version and date (e.g. Legacy devices).
                $deviceVersion = $null
                $deviceDate    = $null
...
function Parse-WiFiDownloadList {
    ...
    # Returns @{ Version, Date, Blocks: [ @{ DEVs, SHA256, Link, Backup, Version, Date } ] }
    # Version/Date in each block are optional (legacy devices only).
    # If absent, the caller should fall back to the global result.Version / result.Date.
    # Mirrors Parse-BTDownloadList structure for consistency.
    $result = @{
        Version = $null
        Date    = $null
    ...
    try {
        $blocks = $Content -split "`n`n" | Where-Object { $_.Trim() }
        ...
        foreach ($block in $blocks) {
            ...
            # Parse global header (Name + DriverVer)
            if ($hasDriverVer) {
                foreach ($line in $lines) {
            ...
            if ($hasDEV) {
                $blockData = @{
                    ...
                    Link    = $null
                    Backup  = $null
                    Version = $null   # optional per-block override (legacy devices)
                    Date    = $null   # optional per-block override (legacy devices)
                }
...
function Get-WiFiBlockForDevice {
    ...
    # Find a per-DEV block matching the detected device ID
...
function Parse-BTDownloadList {
    ...
    # Returns @{ Version, Date, Blocks: [ @{ HWIDs, SHA256, Link, Backup, Version, Date } ] }
    # Version/Date in each block are optional (legacy devices only).
    # If absent, the caller should fall back to the global result.Version / result.Date.
    $result = @{
        Version = $null
...
try {
    ...
    if ($wifiDevice) { $wifiDlContent     = Get-RemoteContent -Url $wifiDownloadUrl }
    if ($btDevice)   { $btDlContent       = Get-RemoteContent -Url $btDownloadUrl }

    if (($wifiDevice -and (-not $wifiLatestContent -or -not $wifiDlContent)) -or
        ($btDevice   -and (-not $btLatestContent   -or -not $btDlContent))) {
    ...
    $wifiDlData = if ($wifiDlContent)     { Parse-WiFiDownloadList -Content $wifiDlContent }   else { $null }
    $btDlData   = if ($btDlContent)       { Parse-BTDownloadList   -Content $btDlContent }     else { $null }
    ...
    if ($wifiDevice -and $wifiData -and $wifiDlData) {
        ...
        if ($devEntry) {
            ...
            # Find per-DEV block in download list (may be $null for devices using global block)
            $wifiBlock = Get-WiFiBlockForDevice -DeviceDEV $wifiDevice.DEV -WiFiDownloadData $wifiDlData

            # Per-device version/date priority:
            # 1. Per-device from .md (columns 5 & 6)
            # 2. Per-block from download list (Version = / Date = fields)
            # 3. Global from .md header
            ...
            } elseif ($wifiBlock -and $wifiBlock.Version) {
                $wifiDeviceVersion = $wifiBlock.Version
                $wifiDeviceDate    = if ($wifiBlock.Date) { $wifiBlock.Date } else { $wifiData.ReleaseDate }
            }
    ...
    if ($btDevice -and $btData -and $btDlData) {
        ...
        if ($pidEntry) {
            ...
            $btBlock = Get-BTBlockForDevice -DevicePID $btDevice.PID -BTDownloadData $btDlData
            ...
            } elseif ($btBlock -and $btBlock.Version) {
                $btDeviceVersion = $btBlock.Version
                $btDeviceDate    = if ($btBlock.Date) { $btBlock.Date } else { $btData.ReleaseDate }
            }
    ...
    # Resolve download block: per-DEV block if available, else first global block
    $wifiDownloadBlock = $null
    if ($wifiInfo) {
        if ($wifiInfo.Block) {
            $wifiDownloadBlock = $wifiInfo.Block
        } elseif ($wifiDlData.Blocks.Count -gt 0) {
            $wifiDownloadBlock = $wifiDlData.Blocks[0]
        }
    }

    if ($wifiNeedsUpdate -and $wifiInfo -and $wifiDownloadBlock -and $wifiDownloadBlock.Link) {
        $wifiResult = Install-DriverWithFallback `
            -PrimaryUrl       $wifiDownloadBlock.Link `
            -BackupUrl        $wifiDownloadBlock.Backup `
            -ExpectedHash     $wifiDownloadBlock.SHA256 `
        ...
        Write-Host " Please check intel-wifi-drivers-download.txt for missing entries." -ForegroundColor Yellow
    }

    # Install Bluetooth driver
    if ($btNeedsUpdate -and $btInfo -and $btInfo.Block) {
        ...
        Write-Host " Please check intel-bt-drivers-download.txt for missing entries." -ForegroundColor Yellow
    }
